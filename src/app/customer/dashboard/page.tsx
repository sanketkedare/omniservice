...
export default function CustomerDashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // 1. User name from local storage or session
      try {
      ...
      // 2. Real active service requests from MongoDB
      ...
      // 3. Real registered properties from MongoDB
  ...
  const defaultProperty = properties.length > 0 ? properties[0] : null;

  return (
    <div className="max-w-7xl mx-auto w-full px-4 sm:px-8 lg:px-12 py-6 sm:py-8 space-y-6 sm:space-y-8">
      {/* Header */}
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              ...
              variant="outline"
              onClick={loadData}
              leftIcon={<RefreshCw className={`h-3.5 w-3.5 ${loadingRequests || loadingProperties ? "animate-spin" : ""}`} />}
            >
              Sync
            </Button>
      ...
      <div className="relative overflow-hidden rounded-3xl border-2 border-orange-300 bg-gradient-to-r from-[#fff5eb] via-white to-[#ffeedb] p-6 sm:p-8 text-neutral-900 shadow-md shadow-orange-950/5">
        <div className="relative z-10 max-w-2xl space-y-3">
          ...
          <p className="text-xs sm:text-sm text-neutral-600 leading-relaxed">
            Record a 15-second video or snap photos. Our AI analyzes root causes, specifies OEM parts, generates an exact Scope of Work, and calculates transparent fair-market pricing for Greater Hyderabad.
          </p>
          <div className="pt-2">
            <Link href="/customer/new-request">
              <Button
      ...
      {/* Active Requests & HomePass Grid (Responsive 1 -> 3 cols) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Active Jobs / Requests (2 cols on lg) */}
        <div className="lg:col-span-2 space-y-4">
          ...
          {loadingRequests ? (
            <div className="rounded-2xl border-2 border-orange-100 bg-white p-8 text-center">
              <p className="text-xs text-neutral-500">Checking active service records in Hyderabad...</p>
            </div>
          ) : requests.length > 0 ? (
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-semibold text-sm text-[#2d130a]">
                            {req.title}
                          </span>
        ...
        {/* HomePass Digital Passport (1 col on lg) */}
        <div className="space-y-4">
          ...
          {loadingProperties ? (
            <div className="rounded-2xl border-2 border-orange-100 bg-white p-6 text-center text-xs text-neutral-500">
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm text-[#2d130a] line-clamp-1">
                      {defaultProperty.name || "My Home"}
                    </CardTitle>
                  </div>
                  <Badge variant="brand" size="sm">
                    Verified
              ...
              <CardContent className="space-y-3 pt-0">
                <div className="space-y-1">
                  <div className="flex justify-between text-xs text-neutral-500">
                    ...
                    <span className="font-bold text-emerald-600 font-mono">
                      {defaultProperty.healthScore || 85} / 100
                    </span>
                  </div>
                  <div className="h-1.5 w-full rounded-full bg-neutral-100">
                    <div
                      className="h-full rounded-full bg-emerald-500 transition-all duration-500"
                      style={{ width: `${Math.min(100, defaultProperty.healthScore || 85)}%` }}
                    />
                  </div>
                </div>
                ...
                <p className="text-[11px] text-neutral-500 flex items-center gap-1">
                  ...
                  <span className="truncate">
                    {defaultProperty.address?.street
                      ? `${defaultProperty.address.street}, ${defaultProperty.address.city || "Hyderabad"}`
                      : "Greater Hyderabad, Telangana"}
                  </span>
                </p>

                <Link href={`/customer/homepass?propertyId=${defaultProperty._id}`} className="block w-full pt-1">
                  <Button size="sm" variant="secondary" fullWidth>
                    Open HomePass Passport
                  </Button>
                </Link>
              </CardContent>
            </Card>
          ) : (
            <Card className="border-2 border-dashed border-orange-200/80 bg-orange-50/20 p-6 text-center shadow-xs">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-2xl bg-orange-100 text-[#f05a28] mb-2">
                <Home className="h-5 w-5" />
              </div>
              <h4 className="text-xs font-bold text-[#2d130a]">No Registered Property Yet</h4>
              <p className="text-[11px] text-neutral-500 mt-1">
                Register your residential address in Greater Hyderabad to activate your digital maintenance passport.
              </p>
              <div className="pt-3">
                <Link href="/customer/properties">
                  <Button variant="brand" size="sm" leftIcon={<Plus className="h-3.5 w-3.5" />}>
                    Register Property
                  </Button>
                </Link>
              </div>
      ...
      {/* AI Provider Discovery & Real Area Suggestions */}
      <div className="pt-2">
