...
const proNavItems: ProNavItem[] = [
  ...
  { label: "Jobs", href: "/pro/jobs", icon: Briefcase },
  { label: "Leads", href: "/pro/leads", icon: Zap, badge: "Live" },
  { label: "Route", href: "/pro/route", icon: Navigation },
...
export function ProfessionalNav({ className }: { className?: string }) {
  ...
  return (
    <nav
      ...
      className={cn(
        "fixed bottom-0 left-0 right-0 z-40 block sm:hidden bg-white/95 backdrop-blur-md border-t border-neutral-200/80 px-2 py-1.5 text-neutral-900 shadow-lg",
        className
      )}
    >
