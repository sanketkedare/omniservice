...
export function AdminNav({ className }: { className?: string }) {
  ...
  return (
    <aside
      className={cn(
        "flex h-full flex-col border-r border-orange-200/70 bg-white font-serif transition-all duration-300 shadow-sm relative z-30 select-none",
        collapsed ? "w-16" : "w-64",
        className
      )}
      style={{ fontFamily: '"Times New Roman", Times, "Liberation Serif", serif' }}
    >
      {/* Sidebar Header: Brand & Sleek Toggle Button */}
      <div className="flex h-16 items-center justify-between border-b border-orange-200/70 px-3.5 bg-gradient-to-r from-orange-50/80 to-white">
        {!collapsed ? (
          <>
      ...
      <div className="border-t border-orange-200/70 p-3 bg-orange-50/60 space-y-2 text-neutral-800">
        {!collapsed && (
          <div className="flex items-center gap-2 text-[11px] font-bold text-emerald-700">
            <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" />
            <span>Hyderabad Governance</span>
          </div>
        )}
        <a
